...
const CertificationsHeader = memo(({ count }) => (
...
CertificationsHeader.displayName = 'CertificationsHeader';

// Memoized Level Badge Component
const LevelBadge = memo(({ level, getLevelColor }) => (
  <div
    className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-semibold border ${getLevelColor(level)}`}
  >
    {level}
  </div>
));
...
// Memoized Certificate Icon Component
const CertificateIcon = memo(({ IconComponent, isHovered }) => (
...
// Memoized Certificate Content Component
const CertificateContent = memo(({ cert }) => (
...
const CertificateCard = memo(({ cert, index, isHovered, onMouseEnter, onMouseLeave, getLevelColor }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-full bg-gradient-to-br from-gray-900/50 to-black/50 backdrop-blur-sm border border-gray-800/50 rounded-3xl p-8 hover:border-indigo-500/50 transition-all duration-500 hover:shadow-2xl hover:shadow-purple-500/10 hover:-translate-y-2 hover:scale-105">
        ...
        <LevelBadge level={cert.level} getLevelColor={getLevelColor} />
        <CertificateIcon IconComponent={IconComponent} isHovered={isHovered} />
...
const Certifications = () => {
  ...
  // Memoize getLevelColor function
    ...
    <section className="min-h-screen bg-black py-20 px-4 relative overflow-hidden">
      ...
      <div className="max-w-full mx-auto relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12 mb-16">
          <CertificationsHeader count={certifications.length} />
        </div>
